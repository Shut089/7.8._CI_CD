...
---
  ...
  * Не забывайте заводить issue в случае отклонений в поведении системы как приложения, так и в реализации тестов. Если технически невозможно реализовать тест по какой-либо причине, то добавьте issue, оставьте тест падающим. Тест должен присутствовать.
  ...
  1. Создайте новый проект с test suite (используйте `describe()`) с API-тестами для [проекта](https://disk.360.yandex.ru/d/AwdOO2AGpOYVmA).
 ...
 **Важно: не забывайте про использование принципа DRY везде, где это необходимо.** 
 ...
 Краткая инструкция по запуску jar: 
 1. Чтобы запустить JAR-файл, убедитесь, что Java установлена на вашем компьютере.  Команда  java -version 
 2. Откройте командную строку (cmd в Windows, Terminal в Linux/macOS).
 3. Перейдите в папку с JAR-файлом:  cd путь_к_папке
 4. Выполните:  java -jar имя_файла.jar  (замените на реальное имя)